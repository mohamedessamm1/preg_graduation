      ...
      body: SingleChildScrollView(
        child: BlocBuilder<HomeCubit, HomeState>(
          builder: (context, state) {
            return HomeCubit.get(context).homeList.isEmpty ? Center(child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                50.ph,
                Lottie.asset('assets/lottie/empty.json',
                    repeat: true, width: 250.w, height: 250.h),
                Text('No Favourite',style: TextStyle(color: Colors.white,fontSize: 18.sp,fontWeight: FontWeight.bold),)
              ],
            )) :  SizedBox(
                    ...
                    itemBuilder: (context, index) {
                      return InkWell(
                        ...
                        child: ItemGridViewHome(
                          index: index,
                          id: 'fav',
                          title: HomeCubit.get(context).favoriteItems?[index]
                              ["title"],
                          image: HomeCubit.get(context).favoriteItems?[index]
                              ['image'],
                        ),
                      );
                    },
